...
Scenario: Verify at least speficed records exist full table matches full table records
	...
	Given only the following records exist in the Test.TestTableAlpha table in the postgresDb database:
...
| 474A6CA3-FBEF-4C38-8C59-6E6AA18E9503 | Bob Obo       | 3/4/1956      | 250   | False         |
	...
	Then the following records should exist in the Test.TestTableAlpha table in the postgresDb database:
...
| 474A6CA3-FBEF-4C38-8C59-6E6AA18E9503 | 
	Then only the following records should exist in the Test.TestTableAlpha table in the postgresDb database:
| TestTableAlphaId                     | 
| F6CC249B-BC42-46B9-A1CF-8D90690E4210 | 
| D8EE9AFC-9869-47FA-A348-66C5CB144ED9 | 
| A0926EEA-876A-4C25-9004-2A4724D73F0C | 
...
Scenario: Query with BigInt
	...
	Given only the following records exist in the Test.TestTableBigInt table in the postgresDb database:
	| Id | Big |
	| 1  | 100   |
	| 2  | 200   |
...
| A0926EEA-876A-4C25-9004-2A4724D73F0C | 
| 474A6CA3-FBEF-4C38-8C59-6E6AA18E9503 | 
	Then the Test.TestTableAlpha table in the postgresDb database will have 4 rows
...
Scenario: Test Row Count statement with no rows, no rows
	Given only the following records exist in the Test.TestTableAlpha table in the postgresDb database:
| TestTableAlphaId                     | 
	Then the Test.TestTableAlpha table in the postgresDb database will have no rows
...
Scenario: Empty strings treated like nulls
	...
	And the query 'insert into Test.TestTableAlpha (TestTableAlphaId, name, Score) values (gen_random_uuid(), 'Me',null)' is run in the postgresDb database
	Then only the following records should exist in the Test.TestTableAlpha table in the postgresDb database:
| Name | DateOfBirth | Score |
| Me   |               |       |
...
Given the following records exist in the Test.TestTableAlpha table in the postgresDb database:
| DateOfBirth        @date |
| today                   |
| yesterday               |
| tomorrow                   |
...
Then the query "select 'abc' as Col1, 123 as Col2, cast('2021-06-04' as timestamp) as Col3" returns:
| Col1 | Col2 | Col3 @date |
| abc  | 123  | 06/04/2021 |
...
| TightDecimal |
| 1              |
Then only the following records should exist in the Test.TestTableAlpha table in the postgresDb database should fail:
| TightDecimal |
| -1             |
